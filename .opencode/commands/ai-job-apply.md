---
description: Run the drafter-reviewer job application workflow on a job posting (URL or pasted text) - evaluate fit, draft CV + cover letter, critique with a reviewer, compile PDFs, and present the final output.
---
...
You already have `data/candidate-profile.md` and `04-job-evaluation.md` in context from Step 1. **Do not re-read them.**
...
- Read any existing `data/cv/main_*.tex` file as a LaTeX template reference
- Read any existing `data/cover-letters/cover_*.tex` or `data/cover-letters/Cover_*.tex` file as a template reference

### CV (`data/cv/main_<company>.tex`)
- Always in **English**
...
### Cover Letter (`data/cover-letters/cover_<company>_<role>.tex`)
...
- `data/candidate-profile.md`
- `data/behavioral-profile.md` — use this specifically to check whether the cover letter's voice matches the candidate's natural register. A "Collaborator" PI profile, for example, should not be given a combative, solo-hero tone; a "Persuader" profile should not be given over-hedged, apologetic phrasing.
- `.agents/skills/job-application-assistant/03-writing-style.md`
- `.agents/skills/job-application-assistant/04-job-evaluation.md`
...
<CV_DRAFT file="data/cv/main_<COMPANY>.tex">
<INSERT_CV_DRAFT_HERE>
</CV_DRAFT>

<COVER_LETTER_DRAFT file="data/cover-letters/cover_<COMPANY>_<ROLE>.tex">
...
```json
{
  "file": "data/cv/main_<COMPANY>.tex" | "data/cover-letters/cover_<COMPANY>_<ROLE>.tex",
...
```bash
cd data/cv && lualatex -interaction=nonstopmode main_<company>.tex
cd ../../data/cover-letters && xelatex -interaction=nonstopmode cover_<company>_<role>.tex
```
...
**Cover letter (`data/cover-letters/cover_<company>_<role>.pdf`):**
- [ ] Exactly 1 page
...
```bash
cd data/cv && pdftotext -layout main_<company>.pdf main_<company>.txt
```
...
Run the full verification checklist from `data/candidate-profile-export.md` now — this is the **only** verification pass in the workflow. Re-read both files once here to verify final state on disk matches your mental model after the Step 4 and Step 5 edits.

### Verification Checklist
Report pass/fail for each item in the data/candidate-profile-export.md verification checklist (factual accuracy, targeting, consistency, quality).
...
- `data/cv/main_<company>.tex`
- `data/cover-letters/cover_<company>_<role>.tex`
