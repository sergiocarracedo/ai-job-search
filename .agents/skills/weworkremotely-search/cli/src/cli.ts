...
if (args.length === 0 || args[0] === "help" || args[0] === "--help") {
  console.log(`
WeWorkRemotely CLI — search remote job listings

Usage:
  weworkremotely-search search [flags]
  weworkremotely-search detail <id> [flags]
  weworkremotely-search help
...
if (command === "search") {
  ...
  const { query, sort, page, limit, format } = parsed.values
  if (!query) {
